...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	If !Empty(aNota)
		...
		If lPe01Nfe
			...
			//DSM Inicio
			If aNota[4] == "1" // Nota Saida
				// Aglutina/Separa itens no XML
				If FindFunction("faturamento.nf.u_aglutinarSepararItens")
					aParam	:= faturamento.nf.u_aglutinarSepararItens(aProd, aICMS, aICMSST, aIPI, aPIS, aPISST, aCOFINS, aCOFINSST, aISSQN, aCST,;
																		aMed, aArma, aveicProd, aDI, aAdi, aExp, aPisAlqZ, aCofAlqZ, aAnfI,;
																		aComb, aCsosn, aPedCom, aICMSZFM, aFCI,;
																		aAgrPis, aAgrCofins, aICMUFDest,;
																		aIPIDevol, aItemVinc, aLote,;
																		aICMSMono, aBenef, aCredPresum, aDeson,;
																		aTotalItem)
					...
					aFCI		:= aParam[24]
					aAgrPis		:= aParam[25]
					aAgrCofins	:= aParam[26]
					aICMUFDest	:= aParam[27]
					aIPIDevol	:= aParam[28]
					aItemVinc	:= aParam[29]
					aLote		:= aParam[30]
					aICMSMono	:= aParam[31]
					aBenef		:= aParam[32]
					aCredPresum	:= aParam[33]
					aDeson		:= aParam[34]
...
Static Function NfeItem(aProd		, aICMS			, aICMSST	, aIPI			, aPIS	   		, aPISST	 , aCOFINS		, aCOFINSST		, aISSQN		, aCST			,;
	...
	//DSM Inicio
	If !Empty(aProd[16])
		cString += '<vUnCom>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],23,10))+'</vUnCom>'
	Else
